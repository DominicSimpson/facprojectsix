...
###### The project was the most complex and advanced that I have worked on, and brought up many challenges. Understanding how a Game Prototype object hardwires everything together was the first, but the real challenge came in transitioning the first level to the second one. Initially I assumed that the two maps in the JavaScript could be part of one object called Levels, with the first level array element 0 and the second element 1. Instead, after debugging, I realised that the best way to have two levels was to have them as separate objects
